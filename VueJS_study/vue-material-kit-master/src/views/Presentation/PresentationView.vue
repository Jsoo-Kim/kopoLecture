...
const btnClick3 = async () => {
  const resp = await $client.get("/api/hello2?name=taro"); // &name=taro2");​
  myModel.resCode = resp.status;
  myModel.respBody = resp.data;
...
const postFormData = async () => {
  ...
  const resp = await $client.post("/api/formPost", formData, {});  // formData는 POST 요청의 본문(body), {}는 config 객체 (헤더, 인증 정보 등 추가적인 설정 담을 수 있음. 현재 비어 있으므로 아무 설정도 없음)
  myModel.respCode = resp.status;
  myModel.respBody = resp.data;
  ...
  console.log("--user--");
  console.log(resp.data.config.data.name);
};
