$(document).ready(function() {
    ...
    $('#userForm').on('submit', function(e) {
        ...
        const userInputs = $('#userInputs input[type="text"]'); // 반환되는 userInputs는 jQuery 객체
        for (let i = 0; i < userInputs.length; i++) {
            const inputElement = userInputs[i];  // DOM 요소 가져옴
            formData['user' + (i + 1)] = $(inputElement).val();
        }      
        
        
        $.ajax({
            url: 'input_action',  // 상대 경로로 요청
            method: "POST", 
            data: formData, 
            success: function(response) {
                $('#message').show(); // 메시지 표시
                setTimeout(function() {
                    window.location.href = '/';  // 루트로 리다이렉트
                }, 500);
            },
            error: function() {
                alert('제출 실패');
