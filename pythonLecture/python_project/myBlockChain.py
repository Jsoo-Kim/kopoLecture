...
def generateGenesisBlock():
    ...
    timestamp = time.time()
    print("time.time() => %f \n" % timestamp)
    tempHash = calculateHash(0, '0', timestamp, "Genesis Block", 0) # 첫 블럭은 채굴 없이 바로 블록 객체 생성?!
    print(tempHash)
    return Block(0, '0', timestamp, "Genesis Block",  tempHash,0)

def calculateHash(index, previousHash, timestamp, data, proof): # SHA256 기반
...
def readBlockchain(blockchainFilePath, mode = 'internal'): # 내부적으로 호출하므로 internal
    print("readBlockchain")
    importedBlockchain = []

    try:
        with open(blockchainFilePath, 'r',  newline='') as file: # 블록체인파일 상대경로, 읽기모드, 개행문자
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName): # 난이도, 읽을 파일
    blockchain = readBlockchain(blockchainPath)  # 10번까지 있으면 1번부터 10번까지의 블록체인 정보를 리스트 형태로 담음?
    strTxData = getTxData() # 트랜잭션 데이터
    if strTxData == '' : # 트랜잭션 데이터가 없다는 건 채굴 대상이 없다는 것!
        print('No TxData Found. Mining aborted')
        return

    timestamp = time.time() # 현재 시각
    proof = 0 # 난이도 만족할 때까지 증가할 예정
    newBlockFound = False
            ...
            if None != re.search('/block/getBlockData', self.path):
               ...
               # 블록 요청 from에 음수값, 0값 예외 처리
                ...
                try:
                    ...
                    block = readBlockchain(g_bcFileName, mode = 'external') # 왼쪽 전역변수는 blockchain.csv라는 파일을 찾음, 오른쪽 파라미터: 기본값은 internal임
                    if block == None:
                    ...
                    self.send_response(500) # 서버 잘못
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    data.append("error")
                finally: # 예외가 났든 안 났든 무조건 실행
                ...
                self.end_headers()
                # mine을 넣어줌
                t = threading.Thread(target=mine) # 왜 이 타이밍에 스레드를 돌까? => 블록 생성에는 시간이 듦! => 락이 안 걸리게 비동기적으로 처리해야 함! => 자원(스레드) 추가
                t.start() # 별도 스레드로 분리돼서 구동됨
