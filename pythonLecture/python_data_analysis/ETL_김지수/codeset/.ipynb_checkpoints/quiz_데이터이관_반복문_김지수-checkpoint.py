...
# 현재 코드 작성 중인 곳으로 들어감
get_ipython().run_line_magic('pwd', '# 현재 위치 codeset')
...
for file in fileList:
    if (file[ : preFixLen] == preFix and file[ -endFixLen : ] == endFix):
        filename = f"{file[ : -4]}_{formatted_datetime}" # 여기에서 파일 저장명 재설정
        # print(filename)
        inData = pd.read_csv(f"../dataset/{file}") # 파일 추출
        inData.to_csv(f"../dataset_out/{filename}_{formatted_datetime}.csv", index=False) # 파일 저장
